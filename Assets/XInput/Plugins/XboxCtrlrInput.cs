	...
	public static class XCI
	{
		...
		public static bool GetButton(XboxButton button)
		{
			...
			if(OnWindowsNative())
			{
				...
				for (int i = (int)XboxController.First; i < (int)XboxController.Count; ++i)
				{
					GamePadState ctrlrState = XInputGetPaticularState(i);
			...
			else
			{
				for (int i = (int)XboxController.First; i < (int)XboxController.Count; ++i)
				{
					string btnCode = DetermineButtonCode(button, i);
		...
		public static bool GetButtonDown(XboxButton button)
		{
			...
			if(OnWindowsNative())
			{
				...
				{
					GamePadState ctrlrState = XInputGetPaticularState(i);
					GamePadState ctrlrStatePrev = XInputGetPaticularStatePrev(i);
		...
		public static bool GetDPad(XboxDPad padDirection)
		{
			...
			{				
				...
				for (int i = (int)XboxController.First; i < (int)XboxController.Count; ++i)
				{
					...
					if (OnMac())
					{
						inputCode = DetermineDPadMac(padDirection, i);
						r = Input.GetKey(inputCode);
					}
					else if (OnLinux() && IsControllerWireless(i))
					{
						inputCode = DetermineDPadWirelessLinux(padDirection, i);
						r = Input.GetKey(inputCode);
					}
					else // Windows Web Player and Linux Wired Controller
					{
						inputCode = DetermineDPad(padDirection, i);
		...
		public static float GetAxis(XboxAxis axis)
		{
			...
			if(OnWindowsNative())
			{
				...
				for (int i = (int)XboxController.First; i < (int)XboxController.Count; ++i)
				{
					...
					{
						result = XInputGetAxisState(ctrlrState.Triggers, axis);
					}
					else
					{
						result = XInputGetAxisState(ctrlrState.ThumbSticks, axis);
					}

					result = XInputApplyDeadzone(result, axis, (XboxController)i);
					if (Mathf.Abs(result) > Mathf.Abs(r))
						r = result;
				}
			}
			else
			{
				for (int i = (int)XboxController.First; i < (int)XboxController.Count; ++i)
				{
					string axisCode = DetermineAxisCode(axis, i);
					float result = 0.0f;

					result = Input.GetAxis(axisCode);
					result = AdjustAxisValues(result, axis, i);
		...
		public static float GetAxisRaw(XboxAxis axis)
		{
			...
			else
			{
				for (int i = (int)XboxController.First; i < (int)XboxController.Count; ++i)
				{
					...
					float result = Input.GetAxisRaw(axisCode);
					result = AdjustAxisValues(result, axis, i);
